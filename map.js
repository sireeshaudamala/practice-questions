...
export const squaresOf = (numbers) =>
  numbers.map((element) => element * element);

// console.log(squaresOf([1, 2, 3]));

// lengths of ["apple", "banana", "kiwi"] => [5, 6, 4]
const lengthsOf = (strings) => strings.map((element) => element.length);

// console.log(lengthsOf(["apple", "banana", "kiwi"]));

// uppercase of ["hello", "world"] => ["HELLO", "WORLD"]
const uppercaseOf = (strings) =>
  strings.map((element) => element.toUpperCase());

// console.log(uppercaseOf(["hello", "world"]));

// first characters of ["apple", "banana", "kiwi"] => ["a", "b", "k"]
const firstCharactersOf = (strings) => strings.map((element) => element.at(0));

// console.log(firstCharactersOf(["apple", "banana", "kiwi"]));

// truth values of [0, 1, 2, 3] => [false, true, true, true]
// Assume non-zero numbers are true, and zero is false
const truthValuesOf = (numbers) => numbers.map((number) => number !== 0);

// console.log(truthValuesOf([0, 1, 2, 3]));

// reverse strings of ["hello", "world"] => ["olleh", "dlrow"]
const reversedStringsOf = (strings) =>
  strings.map((element) => element.split("").reverse().join(""));
...
const noOfVowels = function (initial, char) {
  ...
  return vowels.includes(char) ? initial + 1 : initial;
};

const countVowelsOf = function (strings) {
  return strings.map(function (element) {
    return element.split("").reduce(noOfVowels, 0);
  });
};
...
const removeVowels = function (initial, char) {
  const vowels = ["a", "e", "i", "o", "u"];
...
const withoutVowelsOf = function (strings) {
  return strings.map(function (element) {
    return element.split("").reduce(removeVowels, "");
  });
};
...
const getAdd = function (initial, element) {
  if (initial.length === 0) {
...
const reversedWordsOf = function (strings) {
  return strings.map(function (element) {
    return element.split(" ").map(function (mapper) {
      console.log(mapper);
      return mapper.split("").reverse().join("");
    });
  });
};
...
const isUniqueChar = function (init, element) {
  return init.includes(element) ? init : init + element;
};

const uniqueCharactersOf = function (strings) {
  return strings.map(function (mapper) {
    return mapper.split("").reduce(isUniqueChar, "");
  });
};
// console.log(uniqueCharactersOf(["apple", "banana", "grape"]));

// generate ranges from [3, 5, 2] => [[0, 1, 2], [0, 1, 2, 3, 4], [0, 1]]
const rangesOf = (numbers) => {
  let sub = -1;
  return numbers
    .map((number) => Array(number).fill(0))
    .map((num) => num + (sub += 1));
};
