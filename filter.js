...
const filterRecentOrders = function (orders) {
  return orders.filter(function (order) {
    const [year, month, date] = order.orderDate.split("-");
    return (
      +year === 2024 &&
      ((+month === 11 && +date >= 22 && +date <= 30) ||
        (+month === 12 && +date <= 22 && +date >= 1))
    );
  });
};
...
const filterBelowAveragePrice = (products) => {
  const people = products.map((product) => product.price);
  const total = people.reduce(add, 0);
  const average = total / products.length;

  return products.filter((item) => item.price < average);
};

// active users who posted in the last 7 days [{username: "alice", lastPostDate:
//  "2024-12-01", active: true}, {username: "bob", lastPostDate:
//  "2024-11-20", active: true}] => [{username: "alice",
//  lastPostDate: "2024-12-01", active: true}]
const filterRecentActiveUsers = function (users) {
  return users.filter(function (user) {
    const [year, month, date] = user.lastPostDate.split("-");
    return +year === 2024 && +month === 12 && +date >= 15 && +date <= 22;
  });
};
...
// , {name: "Bob", birthDate: "2024-11-01"}] => [{name: "Alice",
// birthDate: "2024-12-01"}]

const filterBirthdaysThisMonth = function (people) {
  return people.filter(function (person) {
    const [year, month, date] = person.birthDate.split("-");
    return person.birthDate.includes(+year === 2024 && +month === 12);
  });
};
...
const filterHighValueOrders = (orders) => {
  const people = orders.map((order) => order.amount);
  const total = people.reduce(add, 0);
  const average = total / orders.length;

  return orders.filter((item) => item.amount > average);
};

// books with reviews higher than the average rating [{title: "Book 1",
// rating: 4}, {title: "Book 2", rating: 5}, {title: "Book 3", rating: 3}]
// => [{title: "Book 2", rating: 5}]
const filterTopRatedBooks = (books) => {
  const people = books.map((book) => book.rating);
  const total = people.reduce(add, 0);
...
// [{name: "Alice", salary: 5000, department: "HR"}, {name: "Bob", salary:
//  7000, department: "HR"}, {name: "Charlie", salary: 4000, department: "IT"}]
...
// {postId: 2, likes: 200}, {postId: 3, likes: 150}]
// => [{postId: 2, likes: 200}]
const filterPopularPosts = (posts) => {
  const people = posts.map((person) => person.likes);
  const total = people.reduce(add, 0);
...
const filterActiveUsersByPostCount = (users) => {
  const people = users.map((mapper) => mapper.postCount);
  const total = people.reduce(add, 0);
  const average = total / users.length;

  return users.filter((element) => element.postCount > average);
};
...
// filter products that are cheaper than a given price [{name: "item1",
// price: 20}, {name: "item2", price: 50}, {name: "item3", price: 10}]
...
//  {math: 70, science: 85}}] => [{name: "John", grades:
//  {math: 80, science: 90}}]
const filterByMathGrade = (students, grade) =>
  students.filter((student) => student.grades.math > grade);
...
const filterByDate = function (events, date) {
  const [givenYear, givenMonth, givenDate] = date.split("-");
  return events.filter(function (event) {
    const [year, month, date] = event.date.split("-");
    return +givenYear <= +year && +givenMonth <= +month && +givenDate < +date;
  });
};

// filter employees who earn more than a certain salary
// [{name: "Alice", salary: 5000}, {name: "Bob", salary: 7000}]
// => [{name: "Bob", salary: 7000}]
const filterBySalary = (employees, salary) =>
...
const filterByQuantity = (orders, quantity) =>
  orders.filter((order) => order.quantity > quantity);

// filter books published after a certain year [{title: "Book1", year: 2020},
//  {title: "Book2", year: 2022}] => [{title: "Book2", year: 2022}]
const filterByYear = (books, year) => books.filter((book) => book.year > year);
...
const filterBySubjectGrade = (students, subject, threshold) =>
  students.filter((student) => student.grades[subject] > threshold);

// filter photos with a minimum number of likes [{id: 1, likes: 100}, {id: 2, likes: 50}] => [{id: 1, likes: 100}]
const filterByLikes = (photos, likes) =>
  photos.filter((photo) => photo.likes >= likes);
...
const filterByPostCount = (users, postCount) =>
  users.filter((user) => user.posts === postCount);
...
const filterDiscountedItems = (items, discount, maxPrice) =>
  items.map((item) => item.price - discount < maxPrice);
...
const filterLongProductNames = (products, minLength) =>
  products.filter((item) => item.name.toUpperCase().length > minLength);
...
const filterAgeGroups = (users, ageGroup) =>
  users.filter((user) => user.age === ageGroup);
...
const filterPassingGrades = (students, passingGrade) =>
  students.filter((student) => student.grade > passingGrade);
...
const filterHighPriceWithVAT = (products, vatRate, threshold) =>
  products
    .flatMap((item) => [{ name: item.name, price: item.price * (1 + vatRate) }])
...
const filterLongNames = (people, minLength) =>
  people.filter((person) => person.name.length > minLength);
